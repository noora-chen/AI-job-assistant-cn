...
MANUAL_JOBS_PATH = os.path.join(OUTPUT_DIR, "manual_jobs.json")
...
def load_manual_jobs() -> pd.DataFrame:
    """
    Load jobs entered by hand via manual_entry.html (saved as manual_jobs.json
    in the repo root). This is how BOSS直聘 / 智联招聘 / 猎聘 / 前程无忧 —
    platforms we deliberately do NOT auto-scrape — get into the same pipeline
    as the jobspy (LinkedIn/Indeed) results: you paste the job in the browser,
    it gets committed to manual_jobs.json, and this function normalises it to
    look exactly like a jobspy row so filter_jobs()/score/dedup all just work.
    """
    ...
    try:
        ...
        print(f"  [!] Could not read manual_jobs.json: {exc}")
        return pd.DataFrame()
    ...
    for r in records:
        rows.append({
            ...
            "job_url":     r.get("job_url", "") or r.get("url", ""),
            "description": r.get("description", ""),
            "date_posted": r.get("date_posted", ""),
            # "site" drives the coloured source badge in the dashboard —
            # boss / zhilian / liepin / 51job / manual (unrecognised source)
            "site":        (r.get("source") or "manual").strip().lower(),
            "query":       "manual",
            "search_city": r.get("location", ""),
        })
    print(f"  Loaded {len(rows)} manually-entered job(s) from manual_jobs.json")
...
.badge-boss     {{ background: #fff0e6; color: #cc5500; }}
.badge-zhilian  {{ background: #e6f0ff; color: #1a56cc; }}
.badge-liepin   {{ background: #f0e6ff; color: #6b21a8; }}
.badge-51job    {{ background: #ffe6ea; color: #c0392b; }}
.badge-manual   {{ background: #f0f0f0; color: #555555; }}
...
def run_search(save: bool = False, make_site: bool = False):
    ...
    if not manual_df.empty:
        raw_df = pd.concat([raw_df, manual_df], ignore_index=True) if not raw_df.empty else manual_df
